...
if __name__ == "__main__":
    ...
    if args.ae_dir:
        ...
        if ae_args.waveae:
            ...
            ae_model = get_mel_ae(ae_args.encoder_dims, ae_args.latent_size, testbatch['audio'].shape[-1])
    ...
    for i in tqdm.tqdm(range(args.epochs)):
        train_loss = model.train_epoch(loader=train_loader, recon_loss=recon_loss, optimizer=optimizer, device=device, param_loss_w=args.p_w, enc_w=args.enc_w, ae_model=ae_model)
        valid_losses = model.eval_epoch(loader=valid_loader, recon_loss=recon_loss, device=device, ae_model=ae_model)
